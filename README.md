...
横書きの画像テーマと`post`・`iconquote`では、画像生成時にMarkdownを解釈します。`**太字**`、`*斜体*`、`~~取消線~~`、`` `code` ``、見出し、リンク表示に対応しています。

インライン数式は`$E=mc^2$`、独立した数式行は`$$\\frac{a}{b}$$`の形式です。分数、平方根、上下付き、ギリシャ文字、総和・積分などの主要なTeX記法を、外部CDNなしで直接PNGへ描画します。
